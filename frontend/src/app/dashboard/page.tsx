...
const DEFAULT_CONFIG = {
  pages: [{ id: "home", title: "Dashboard", components: [
...
export default function Dashboard() {
  ...
  useEffect(() => { if (!loading && user) fetchApps(); }, [user, loading]);
  ...
  if (loading) return (
    <div style={{ minHeight: '100vh', background: '#0a0a0f', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ textAlign: 'center' }}>
        ...
        <p style={{ color: '#6b7280' }}>Loading AppGen...</p>
      </div>
    </div>
  );
            ...
            {(['en', 'hi', 'mr'] as const).map(l => (
              <button key={l} onClick={() => setLang(l)}
                style={{ padding: '4px 10px', borderRadius: '6px', border: 'none', cursor: 'pointer', fontSize: '0.75rem', fontWeight: '600',
                  background: lang === l ? '#6366f1' : 'transparent', color: lang === l ? 'white' : '#6b7280' }}>
                {l === 'en' ? 'EN' : l === 'hi' ? 'हि' : 'म'}
              </button>
            ))}
          </div>
      ...
      <div style={{ maxWidth: '1100px', margin: '0 auto', padding: '2rem 1.5rem' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '2rem', flexWrap: 'wrap', gap: '1rem' }}>
          ...
          <button onClick={() => setShowCreate(true)}
            style={{ display: 'flex', alignItems: 'center', gap: '8px', background: '#6366f1', border: 'none', borderRadius: '12px', padding: '0.75rem 1.25rem', color: 'white', fontWeight: '700', cursor: 'pointer', fontSize: '0.9rem' }}>
        ...
        {apps.length === 0 ? (
          <div style={{ background: '#111118', border: '1px solid #2a2a3a', borderRadius: '20px', padding: '4rem', textAlign: 'center' }}>
            ...
            <button onClick={() => setShowCreate(true)} style={{ background: '#6366f1', border: 'none', borderRadius: '10px', padding: '0.7rem 1.5rem', color: 'white', fontWeight: '600', cursor: 'pointer' }}>{t.createApp}</button>
          </div>
        ) : (
            ...
            {apps.map((app) => (
              <div key={app.id} onClick={() => router.push(`/app/${app.id}`)}
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
                  ...
                  <button onClick={(e) => deleteApp(app.id, e)} style={{ background: 'none', border: 'none', color: '#6b7280', cursor: 'pointer', padding: '4px' }}
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ color: '#6b7280', fontSize: '0.75rem', display: 'flex', alignItems: 'center', gap: '4px' }}><Clock size={12} />{new Date(app.created_at).toLocaleDateString()}</span>
                  <div style={{ display: 'flex', gap: '0.5rem', alignItems: 'center' }}>
                    ...
                    <span style={{ color: '#6366f1', fontSize: '0.8rem', display: 'flex', alignItems: 'center', gap: '4px', fontWeight: '600' }}>{t.open} <ChevronRight size={14} /></span>
                  </div>
                </div>
              </div>
            ))}
      ...
      {showCreate && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem', zIndex: 100, backdropFilter: 'blur(4px)' }}>
          <div style={{ background: '#111118', border: '1px solid #2a2a3a', borderRadius: '20px', padding: '2rem', width: '100%', maxWidth: '680px', maxHeight: '90vh', overflow: 'auto' }}>
            ...
            <textarea value={configJson} onChange={e => { setConfigJson(e.target.value); validateJson(e.target.value); }} rows={16}
            ...
            <div style={{ display: 'flex', gap: '0.75rem' }}>
              <button onClick={createApp} disabled={creating || !!jsonError}
                style={{ flex: 1, background: creating || jsonError ? '#1a1a24' : '#6366f1', border: 'none', borderRadius: '12px', padding: '0.85rem', color: 'white', fontWeight: '700', cursor: creating || jsonError ? 'not-allowed' : 'pointer' }}>
